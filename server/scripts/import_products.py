...
PRODUCT_TABLE_RE = re.compile(r"^\|\s*\d+\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*(\d+)\s*\|")
...
def parse_products(md_path: Path):
    ...
    for line in md_path.read_text(encoding="utf-8").splitlines():
        ...
        products.append((scenario_key, name))
    return products
...
def upsert_products(conn, products):
    inserted = 0
    skipped = 0
    for scenario_key, name in products:
...
def main():
    ...
    try:
        ...
        inserted, skipped = upsert_products(conn, products)
        admin_created = ensure_admin(conn, args.admin_user, args.admin_password)
        conn.commit()
    finally:
        conn.close()

    print(f"products inserted={inserted} skipped(existing)={skipped}")
